...
def load_config(config_path: Optional[str] = None, override_path: Optional[str] = None) -> Dict[str, Any]:
    """
    Load configuration from YAML file, with optional override.

    String values may contain ``${ENV_VAR}`` or ``${ENV_VAR:-default}``
    placeholders that are resolved from the process environment.

    Args:
        config_path: Path to base config YAML. If None, uses config/config.yaml.
        override_path: Path to a partial YAML that is deep-merged on top of the
            base config. Only the keys present in the override file are changed;
            all other settings inherit from the base config.
    ...
    if override_path is not None:
        ...
        override = _interpolate_env_vars(override)
