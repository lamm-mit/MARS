                ...
                for points in points_list:
                    ...
                    if label_type == 'image':
                        try:
                            ...
                            cropped.save(os.path.join(output_path, 'images', f"{jdx}_{img_idx}.jpg"))
                        except Exception as e:
                            print(e)
                            pass
                        img_idx += 1
...
def process_pdf(input_path, output_path):
    ...
    Args:
        ...
        output_path: Path to output directory (will be created if needed)
    ...
    try:
        ...
        print(f'{Colors.RED}PDF loading: {input_path}.....{Colors.RESET}')
        ...
        mmd_det_path = os.path.join(output_path, pdf_filename.replace('.pdf', '_det.mmd'))
        mmd_path = os.path.join(output_path, pdf_filename.replace('.pdf', '.mmd'))
        pdf_out_path = os.path.join(output_path, pdf_filename.replace('.pdf', '_layouts.pdf'))
        ...
        print(f'{Colors.GREEN}Successfully processed: {input_path}{Colors.RESET}')
        return True
        
    except Exception as e:
        print(f'{Colors.RED}Error processing {input_path}: {str(e)}{Colors.RESET}')
        import traceback
        traceback.print_exc()
        return False
