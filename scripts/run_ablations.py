...
Usage:
    ...
    python scripts/run_ablations.py --queries Query1                 # specific query
...
def _init_rag_resources(config):
    ...
    def _load_col(db_cfg, label):
        ...
        _cols = cl.list_collections()
        cn = db_cfg.get("collection_name") or (_cols[0] if isinstance(_cols[0], str) else _cols[0].name)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Root output directory (default: results, or results_openai when running 1agent_no_rag_openai)",
    )
    parser.add_argument(
        "--openai-key", default=None,
        help="OpenAI API key for the 1agent_no_rag_openai condition (default: OPENAI_API_KEY env var)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--openai-max-tokens", type=int, default=40000,
        help="Max output tokens for OpenAI calls (default: 40000)",
    )
    parser.add_argument(
    ...
    # Default output dir: results_openai when the only condition is the openai one
    _openai_conditions = {"1agent_no_rag_openai"}
    _has_openai = any(c in _openai_conditions for c in conditions)
    _has_local = any(c not in _openai_conditions for c in conditions)
    if args.output_dir is None:
        output_dir = "results_openai" if (_has_openai and not _has_local) else "results"
    else:
        output_dir = args.output_dir

    # Configured LLM (used by all non-openai conditions)
    llm_cfg = config["llm"]
    generate = None
    if _has_local:
        llm_instance = llm({
    ...
    # OpenAI LLM (needed only for 1agent_no_rag_openai)
    openai_generate = None
    if _has_openai:
        api_key = args.openai_key or os.environ.get("OPENAI_API_KEY", "")
        if not api_key:
            print("ERROR: OpenAI API key required for 1agent_no_rag_openai.")
            print("  Pass --openai-key or set OPENAI_API_KEY.")
            sys.exit(1)
    ...
    print(f"Model:      {configured_model} ({_backend})" + (f"  |  {args.openai_model} (OpenAI API)" if _has_openai else ""))
    ...
    for cond in conditions:
        for i, query in enumerate(queries, 1):
            name = query["name"]
            out_dir = str(PROJECT_ROOT / output_dir / name)
            ...
            elif cond == "1agent_no_rag_openai":
                result = run_1agent_no_rag_ablation(
                    query, openai_generate, ablation_prompts, temperature
                )
                result["metadata"]["ablation_condition"] = "1agent_no_rag_openai"
                result["metadata"]["openai_model"] = args.openai_model
            else:
                continue
