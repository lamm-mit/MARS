...
Performs blind pairwise evaluation of MARS baseline vs four ablation conditions
...
Usage:
    ...
    python scripts/run_evaluation.py [--model gpt-4o] [--queries Query1] [--output-dir evaluation_results]
...
SYSTEM_LABELS = {
    ...
    "ablation_1agent_no_rag_openai": "1-Agent (No RAG/KG, OpenAI)",
}
...
def _collect_query_dirs_under(results_dir: Path) -> Dict[str, Path]:
    """Scan one directory for QueryN/ folders that contain all 5 condition files."""
    ...
    for d in sorted(results_dir.iterdir()):
        ...
        has_1norag_openai = (d / "ablation_1agent_no_rag_openai.json").exists() or any(
            f.name.startswith("ablation_1agent_no_rag_openai_") and f.suffix == ".json" for f in d.iterdir()
        )
        if has_mars and has_3agent and has_1rag and has_1norag and has_1norag_openai:
            results[query_name] = d
    return results


def discover_query_dirs(results_root: Optional[Path] = None) -> Dict[str, Path]:
    """Find query directories that have all 5 condition files.
...
def load_all_conditions(query_dir: Path) -> Dict[str, Dict[str, Any]]:
    """Load all 5 condition files for a query from ``query_dir``."""
    return {
        ...
        "ablation_1agent_no_rag_openai": _load_json_file(
            _find_condition_file(
                query_dir,
                "ablation_1agent_no_rag_openai.json",
                "ablation_1agent_no_rag_openai_",
            )
        ),
    }
...
def build_judge_prompt(
    ...
    all_labels = sorted(blind_mapping.keys())
    n_systems = len(all_labels)
    label_str = ", ".join(all_labels)

    system_prompt = (
        f"You are an expert materials scientist acting as a blind evaluator. "
        f"You will receive outputs from {n_systems} anonymized systems (labeled {label_str}) "
    ...
    ranking_placeholders = ", ".join(f'"<#{i+1} label>"' for i in range(n_systems))
    json_schema = "{\n"
    for label in all_labels:
        json_schema += f'  "{label}": {{\n'
        ...
        json_schema += "  },\n"
    json_schema += f'  "ranking": [{ranking_placeholders}],\n'
...
def _clamp_parsed_scores(parsed: Dict[str, Any], rubric: Dict[str, Any]) -> None:
    ...
    for label in [k for k in parsed if len(k) == 1 and k.isalpha()]:
...
def evaluate_query(
    ...
    active_keys = list(systems.keys())
    query_sentence = systems["evaluation"]["query"]["sentence"]

    # Randomize label assignment for blind evaluation
    shuffled_keys = active_keys[:]
    random.shuffle(shuffled_keys)
    blind_labels = [chr(ord("A") + i) for i in range(len(active_keys))]
    blind_mapping = {label: key for label, key in zip(blind_labels, shuffled_keys)}
    reverse_mapping = {v: k for k, v in blind_mapping.items()}

    print(f"  Query: {query_sentence[:100]}...")
    print(f"  Blind mapping (hidden from judge): {json.dumps({v: SYSTEM_LABELS.get(k, k) for v, k in sorted(blind_mapping.items())})}")
    ...
    unblinded_scores = {}
    for condition_key in active_keys:
...
def print_aggregate_summary(all_results: List[Dict[str, Any]], rubric: Dict[str, Any]):
    ...
    # Derive active conditions from the first result
    active_keys = list(all_results[0]["scores"].keys())

    # Collect scores per condition
    totals = {ck: {d: [] for d in dimensions} for ck in active_keys}
    rank_positions = {ck: [] for ck in active_keys}
    ...
    # Win/loss table
    n_pos = len(active_keys)
    pos_headers = "".join(f"{f'#{i+1}':>6}" for i in range(n_pos))
    print(f"\n{'System':<35} {pos_headers}")
    print(f"{'-'*35} " + " ".join([f"{'-'*6}"] * n_pos))
    for ck in active_keys:
        label = SYSTEM_LABELS.get(ck, ck)
        ranks = rank_positions[ck]
        counts = [ranks.count(i) for i in range(1, n_pos + 1)]
        print(f"{label:<35} " + "".join(f"{c:>6}" for c in counts))
...
def main():
    ...
    if not query_dirs:
        ...
        print(f"ERROR: No query directories found with all 5 condition files{hint}.")
        sys.exit(1)
    ...
    if valid_results:
        dimensions = list(rubric["dimensions"].keys())
        active_keys = list(valid_results[0]["scores"].keys())
        agg_scores = {}
        for ck in active_keys:
            agg_scores[ck] = {"label": SYSTEM_LABELS.get(ck, ck)}
            for dim in dimensions:
